...
An alternative to selecting invidually matched controls is to create a sample controls, controls that are not individually linked to cases. This can be done using the `createSamplingCriteria` instead of the `createMatchingCriteria` function. For example:
...
We can now fit the model. In case we used matchign to select controls this will be a logistic regression conditioned on the matched sets. Else it will be an unconditioned logistic regression.
