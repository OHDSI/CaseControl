...
Before installing the `CaseControl` package make sure you have Java available.  Java can be downloaded from [www.java.com](http://www.java.com). For Windows users, RTools is also necessary.  RTools can be downloaded from [CRAN](http://cran.r-project.org/bin/windows/Rtools/). See [these instructions](https://ohdsi.github.io/Hades/rSetup.html) for properly configuring your R environment.

The `CaseControl` package is currently maintained in a [Github repository](https://github.com/OHDSI/CaseControl), and has dependencies on other packages in Github. All of these packages can be downloaded and installed from within R using the `drat` package:
...
data.frame(cohort_definition_id = c(1,2),count = c(1225890, 285459))
```
...
Data about the cases and potential controls are extracted from the server and stored in the `caseData` object. This object uses the package `Andromeda` to store information in a way that ensures R does not run out of memory, even when the data are large. 
...
Creating the `caseData` object can take considerable computing time, and it is probably a good idea to save it for future sessions. Because `caseData` uses `Andromeda`, we cannot use R's regular save function. Instead, we'll have to use the `saveCaseData()` function:

```{r eval=FALSE}
saveCaseData(caseData, "GiBleed.zip")
```
...
Next, we can use the data to select controls. In this example we'll selected matched controls, meaning that for every case we'll find individually matched controls:
...
An alternative to selecting individually matched controls is to create a sample controls, controls that are not individually linked to cases. This can be done using the `createSamplingCriteria` instead of the `createMatchingCriteria` function. For example:
...
When including covariates, the `caseControlsExposure` object uses `Andromeda` to store large objects and therefore cannot be stored using standard functions. Please use `saveCaseControlsExposure` to save:
...
We can now fit the model. In case we used matching to select controls this will be a logistic regression conditioned on the matched sets. Else it will be an unconditioned logistic regression.
