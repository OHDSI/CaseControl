...
Next, we load information on the exposures of the cases and controls.
...
Here we specify that we will use the `drug_era` table to identify exposures, and will only retrieve data on exposure to Diclofenac (concept ID 1124300).

The `caseControlsExposure` object is a standard R object and can be saved as usual, for example:

```{r tidy=TRUE,eval=FALSE}
saveRDS(caseControlsExposure, "caseControlsExposure.rds")
```
